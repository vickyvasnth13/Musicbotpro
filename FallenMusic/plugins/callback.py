...
from FallenMusic import YouTube, app, userbot
...
from FallenMusic.utils.stream.autoclear import auto_clean
from FallenMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "UpVote":
        ...
        if get_upvotes >= upvote:
            ...
            if (
                ...
                await CallbackQuery.answer("ᴀᴅᴅᴇᴅ ᴜᴘᴠᴏᴛᴇ.", show_alert=True)
            else:
                await CallbackQuery.answer("ʀᴇᴍᴏᴠᴇᴅ ᴜᴘᴠᴏᴛᴇ.", show_alert=True)
            upl = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text=f"ᴠᴏᴛᴇ {get_upvotes}",
                            callback_data=f"ADMIN  UpVote|{chat_id}_{counter}",
                        )
                    ]
                ]
            )
            await CallbackQuery.answer("ᴜᴘᴠᴏᴛᴇᴅ", show_alert=True)
    ...
    if command == "Pause":
        ...
        await Fallen.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await Fallen.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Fallen.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Fallen.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            button = telegram_markup(_, chat_id)
            img = await gen_thumb(videoid, user_id, original_chat_id)
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Fallen.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
