...
from config import (BANNED_USERS, CLEANMODE_DELETE_MINS,
                    MUSIC_BOT_NAME, OWNER_ID, SUPPORT_GROUP, SUPPORT_CHANNEL)
from strings import get_command
from FallenMusic import app
from FallenMusic.utils.database import (add_nonadmin_chat,
...
from FallenMusic.utils.database.memorydatabase import (
    get_upvote_count,
    is_skipmode,
    set_upvotes,
    skip_off,
...
async def settings_mar(client, message: Message, _):
    ...
    await app.send_photo(message.chat.id,
        image,
        _["setting_1"].format(MUSIC_BOT_NAME, message.chat.id, message.chat.title),
...
async def settings_cb(client, CallbackQuery, _):
    ...
    return await CallbackQuery.edit_message_text(
        _["setting_1"].format(MUSIC_BOT_NAME,
    ...
    await CallbackQuery.edit_message_media(
        InputMediaPhoto(
            ...
            caption=_["start_2"].format(
               CallbackQuery.from_user.first_name, ok
            ),
        ),
        reply_markup=InlineKeyboardMarkup(buttons)
    )
...
@app.on_callback_query(filters.regex(pattern=r"^(AUTH|AUTHLIST)$") &
~BANNED_USERS)
@ActualAdminCB
